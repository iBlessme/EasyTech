...
//        guard let uid = Auth.auth().currentUser?.uid else {return}
//        let ref = Int.random(in: 1..<999999)
//        Firestore.firestore().collection("users").document(uid).collection("orders").document(String(ref)).setData([
//
//            "imageOrder" : "1",
//            "housing" : housing,
//            "floor" : floor,
...
//            }else{
//                print("Документ создан")
//                let refI = Storage.storage().reference().child("orders").child(String(ref))
//
//                guard let imageData = imageOrder.jpegData(compressionQuality: 0.4) else {return}
//                let metadata = StorageMetadata()
//                metadata.contentType = "image/jpeg"
//
//                _ = refI.putData(imageData, metadata: metadata) {metadata, error in
//                    if let error = error{
//                        print("Failed image 1 \(error)")
//                    }
//                    guard let metadata = metadata else {return}
//                    _ = metadata.size
//                    refI.downloadURL{ url, error in
//                        guard let downloadUrl = url else {return}
//                        print(downloadUrl)
//                        print("Success upload image")
//                        Firestore.firestore().collection("users").document(uid).collection("orders").document(String(ref)).updateData([
//                            "imageOrder" : downloadUrl.absoluteURL
//                        ]){err in
//                            print("Не добавлен путь")
//                            return
//                        }
//
//                    }
